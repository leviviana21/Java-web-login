...
public class ServletLogin extends HttpServlet {
	...
	String confirmaSenha = req.getParameter("txtConfirmaSenha");
	...
	Usuario usuarioLogado = banco.login(email, senha);
		if (usuarioLogado != null) {
			resp.getWriter().append("Bem-vindo, " + usuarioLogado.getNome() + "!");
			return;
		} else {
			req.setAttribute("error", true);
			req.setAttribute("msg", "Email ou senha incorretos");
		...
		if (nome == null || email == null || senha == null || confirmaSenha == null || senha.length() < 4 || !senha.equals(confirmaSenha)) {
		    req.setAttribute("error", true);
		    req.setAttribute("msg", "Senhas não batem ou campos incorretos.");
